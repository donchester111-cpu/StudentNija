...
function saveAll() {
  ...
  localStorage.setItem('studentnija_s', JSON.stringify(exams));
...
// ---- Close Exams page and go back to Home ----
...
function renderHome() {
  ...
  const tools = [
    ...
    { name: "Exam Hub", icon: "📝", action: "currentPage='exams'; renderMainApp();" },
  ];
...
`;
    ...
    if (currentPage === 'ai') {
        ...
        pagesContainer.style.display = 'flex';
        pagesContainer.style.flexDirection = 'column';
        // Set exams-page to flex
        const page = document.getElementById('exams-page');
        if (page) {
            page.style.height = '100%';
            page.style.display = 'flex';
            page.style.flexDirection = 'column';
        ...
        backBtn.style.cssText = `
            ...
            transition: 0.2s;
        `;
        backBtn.id = 'examsBackBtn';
        backBtn.onclick = function() {
            window.closeExamsPage();
        };
        const existing = document.getElementById('examsBackBtn');
        if (existing) existing.remove();
        document.body.appendChild(backBtn);
        ...
        // Reset exams page styles
        const exPage = document.getElementById('exams-page');
        if (exPage) {
            exPage.style.display = '';
            exPage.style.height = '';
            exPage.style.flex = '';
            exPage.style.overflow = '';
        }
        const exContent = document.getElementById('examsContent');
        if (exContent) {
            exContent.style.height = '';
            exContent.style.display = '';
            exContent.style.flex = '';
            exContent.style.overflow = '';
        }
        // Remove back buttons if exists
        const backBtn1 = document.getElementById('studyGroupsBackBtn');
        if (backBtn1) backBtn1.remove();
        const backBtn2 = document.getElementById('examsBackBtn');
    ...
    renderExamsPage(); // always render, but hidden unless active
    attachBottomNav();
    checkAchievements();
